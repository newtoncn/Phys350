...
RopeSimulation* ropeSimulation = new RopeSimulation(
	...
	0.0125f,					// Normal Length Of Springs In The Rope
	0.25f,					// Spring Inner Friction Constant
	Vector3D(0, -9.81f, 0), // Gravitational Acceleration
	0.22f,					// Air Friction Constant
	70.0f,					// Ground Repel Constant
	0.3f,					// Ground Slide Friction Constant
	3.0f,					// Ground Absoption Constant
...
void Update(DWORD milliseconds)											// Perform Motion Updates Here
{
	...
	if (g_keys->keyDown[VK_RIGHT] == TRUE)									// Is The Right Arrow Being Pressed?
		ropeConnectionVel.x += move;										// Add Velocity In +X Direction

	if (g_keys->keyDown[VK_LEFT] == TRUE)									// Is The Left Arrow Being Pressed?
		ropeConnectionVel.x -= move;										// Add Velocity In -X Direction

	if (g_keys->keyDown[VK_SPACE] == TRUE)									// Is The Up Arrow Being Pressed?
		ropeConnectionVel.z -= move;										// Add Velocity In +Z Direction

	if (g_keys->keyDown[VK_CONTROL] == TRUE)									// Is The Down Arrow Being Pressed?
		ropeConnectionVel.z += move;										// Add Velocity In -Z Direction

	if (g_keys->keyDown[VK_UP] == TRUE)									// Is The Home Key Pressed?
		ropeConnectionVel.y += move;										// Add Velocity In +Y Direction

	if (g_keys->keyDown[VK_DOWN] == TRUE)									// Is The End Key Pressed?
		ropeConnectionVel.y -= move;										// Add Velocity In -Y Direction

	if (g_keys->keyDown[VK_F2] == TRUE)									// Is The F2 Key Pressed?
		ropeSimulation = new RopeSimulation(
			...
			0.3f,					// Ground Slide Friction Constant
			3.0f,					// Ground Absoption Constant
			-1.0f);					// Height Of Ground										// Add Velocity In -Y Direction
